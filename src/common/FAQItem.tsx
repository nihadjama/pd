...
export default function FAQItem({ question, answer, isOpen, onToggle }: FAQItemProps) {
  return (
    <button
      onClick={onToggle}
      className="border-[#e5e7eb] border-b flex flex-col items-start px-4 md:px-8 lg:px-16 py-4 relative shrink-0 w-full md:w-full text-left"
    >
      <div className="flex items-start relative shrink-0 w-full">
        <div className="flex flex-col grow items-start min-w-0 shrink-0 w-full">
          <div className={`${ isOpen ?  "border-[#e5e7eb]" :"border-transparent"} border-b flex gap-10 items-start px-0 py-5 relative shrink-0 w-full`}>
