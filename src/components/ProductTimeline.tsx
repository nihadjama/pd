    ...
    <div className="w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {timelineData.map((item, index) => (
          <div
            key={item.year}
            className="border border-[#e5e7eb] rounded-xl bg-white p-6 md:p-8 flex flex-col gap-4 hover:border-[#5e48f0] transition-all duration-200"
          >
            {/* Year Badge */}
            <div className="flex items-center gap-3">
              <div className="bg-[rgba(94,72,240,0.1)] border border-[rgba(94,72,240,0.25)] flex items-center justify-center px-4 py-2 rounded-lg shrink-0">
                <p className="font-heading font-semibold text-lg text-[#5e48f0]">
                  {item.year}
                </p>
              </div>
            </div>
            ...
            <ul className="flex flex-col gap-3">
              {item.features.map((feature, featureIndex) => (
                ...
                >
                  <div className="shrink-0 w-1.5 h-1.5 rounded-full bg-[#5e48f0] mt-2" />
